...
- 统一 AV 入参为 `avID`，四个读取/渲染动作保留 `id` 弃用别名并拒绝冲突；修正列查询、默认创建开关和创建回读，保留单选等业务字段并逐格校验批量写入
- 创建文档前先校验路径参数，补齐无 Resources 客户端的排版帮助入口并修复 CLI `--topic`；同步双语文档与 Skills，CLI 包提升至 v0.2.8

## v0.6.6 - 2026-09-09

- 修复思源 3.8.3 在统计、宠物状态和写入账本更新时反复重载插件的问题，保持 HTTP MCP 进程、会话及写入租约稳定
- HTTP 服务运行时默认选择 HTTP/HTTPS，复制配置附近补充多工作空间端口说明，并保留用户手动选择
- stdio 连接配置固定使用 6806，避免重启后随机端口失效；明确其连接第一个启动的工作空间，API Token 须与目标工作空间一致

## v0.6.5 - 2026-09-09

- 连接配置与 HTTP MCP 启动使用当前思源工作区地址及配套凭据，修复多工作区误用 6806 和本机 HTTPS 证书导致的内核连接失败；已复制的客户端配置在工作区端口变化后需重新生成
- 修复数据库筛选的递归 Schema 引用，统一批量更新预检目标计数，将持久化行内属性纳入严格写入状态校验，并消除 AV 属性排列引起的误判
- 优化大文档读取的内存占用、大小上限与续读行为；补充真实思源回归验收脚本，CLI 包同步提升至 v0.2.7
...
- 为内置变更型 action 增加严格安全写入：以 4 位起步的临时预检租约解析完整 SHA-256，并结合 UUIDv7 请求幂等、串行协调与写后读回，拒绝过期状态和重复执行
- 在“设置与调试”中加入默认开启的严格写入开关；不创建思源仓库快照，并补齐 HTTP、stdio、CLI 与真实测试笔记本的全 action 验收
- 修正时间线回退预检与 MCP App 内联预览资源，CLI 包同步提升至 v0.2.4
